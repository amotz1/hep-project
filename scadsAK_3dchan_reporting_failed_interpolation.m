...
% inmat3d = ALLEEG.data; 
function [outmat3d, interpsensvec] = scadsAK_3dchan_reporting_failed_interpolation(inmat3d, locations,subject_index) 
...
%         if all(isnan(trialdata2d(interpvec(badsensor),:)))
%            disp('interpolation failed for sensor')
%            disp(interpvec(badsensor));
       end
               
           outmat3d(:, :, trial) = trialdata2d; % Creates output file where bad channels have been replaced with interpolated data
    
    end    
[rows, ~] = find(isnan(outmat3d));
if size(rows,1) == size(outmat3d(:,:,:),3)* size(outmat3d(:,:,:),2)
    file = fopen('C:\Users\amotz\amotz\for_nava\hep_project\hbd_variables\testingPipeline\error.txt','wt');
    fprintf(file,'interpulation failed %d for subject %d',unique (rows),subject_index);
    fclose(file);
    disp('interpolation failed for sensor')
    disp(unique(rows));
end
